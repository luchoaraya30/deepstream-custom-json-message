...
generate_analytics_module_object (NvDsMsg2pCtx *ctx, NvDsEventMsgMeta *meta)
{
  ...
  //--------------------- CUSTOM CODE -----------------------//
  // Define a new json object to create nested message
  JsonObject *jobject;
  //---------------------------------------------------------//
  ...
  // This first four parameters comes from the msgconv configuration file (see configs/dstest5_msgconv_sample_config.txt)
  // I just want the ID
  ...
  // Object meta
  json_object_set_int_member (analyticsObj, "source_id", meta->source_id);
  json_object_set_int_member (analyticsObj, "frame_id", meta->frame_id);
  json_object_set_int_member (analyticsObj, "object_id", meta->object_id);
  json_object_set_string_member (analyticsObj, "object_class", meta->object_class);
  
  // Bbox params
  jobject = json_object_new ();
  json_object_set_double_member (jobject, "left", meta->left);
  json_object_set_double_member (jobject, "top", meta->top);
  json_object_set_double_member (jobject, "with", meta->width);
  json_object_set_double_member (jobject, "height", meta->height);
  json_object_set_object_member (analyticsObj, "bbox", jobject);
  
  // Classifier meta
  jobject = json_object_new ();
  json_object_set_string_member (jobject, "person_gender", meta->person_gender);
  json_object_set_string_member (jobject, "person_age", meta->person_age);
  json_object_set_string_member (jobject, "car_color", meta->car_color);
  json_object_set_string_member (jobject, "car_type", meta->car_type);
  json_object_set_object_member (analyticsObj, "attributes", jobject);
...
{
  //---------------------- CUSTOM CODE --------------------//
  // Just commented the stuff I didn't need
  ...
  json_object_set_object_member (rootObj, "detection", analyticsObj);
